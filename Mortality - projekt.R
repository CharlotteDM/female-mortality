...
#mortality in 2019
mort2019 <- select(mortality_fem, Country.Name, Country.Code, X2019) #nie zadziałała za 2 razem
#---mort2019 <- mortality_fem[, c("Country.Name", "Country.Code", "X2019")]

#---removing rows with NA
#---mort2019 <- na.omit(mort2019)

#finding country with the highest mortality rate in 2019
max(mort2019$X2019, na.rm = TRUE) #code, when I do not remove NA from data frame
#---max(mort2019$X2019)
...
#---mort2000 <- mortality_fem[, c("Country.Name", "Country.Code", "X2000")]

#removing rows with NA 
#---mort2000 <- na.omit(mort2000)

#finding country with the highest mortality rate in 2000
max(mort2000$X2000, na.rm = TRUE)
the_highest_MR2000 <- filter(mort2000, X2000 == max(X2000, na.rm = TRUE)) #Kiribati - 47.8

#finding country with the lowest mortality rate in 2000
...
#---mortality_fem_2000_2019 <- na.omit(mortality_fem_2000_2019)
...
max(EU$X2019, na.rm = TRUE) #code, when I do not remove NA from data frame
#---max(mort2019$X2019)
...
#scatterplott mortality and GDP(%)
...
physicians2018 <- c(5.2, 3.1, 4.2, 4.1, 4, 4.3, 4.2, 4, 3.5, 3.2, 3.2, 
                     ...
                     2.4, 5.2, 3.1, 3.5, 3.2, 4.3) #ilu lekarzy przypada na 1000 pacjentów
...
#max and min risk of poverty or social exclusion in EU
...
combined_data <- combined_data[, - c(101:103)] #tego nie dawaj już może do projektu bo coś najebałas
combined_data <- combined_data[, -89] #tego też nie dawaj

#Renaming columns in data frame

names(combined_data)[names(combined_data) == "Country.Name.x"] <- "Country.Name" #tego nie pisz w projekcie bo znów najebane
...
names(combined_data)[names(combined_data) == "X2012..YR2012."] <- "educ2012"
names(combined_data)[names(combined_data) == "X2013..YR2013."] <- "educ2013"
names(combined_data)[names(combined_data) == "X2014..YR2014."] <- "educ2014"
names(combined_data)[names(combined_data) == "X2015..YR2015."] <- "educ2015"
names(combined_data)[names(combined_data) == "X2016..YR2016."] <- "educ2016"
names(combined_data)[names(combined_data) == "X2017..YR2017."] <- "educ2017"
